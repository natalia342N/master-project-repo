...
namespace ngla
{
      ...
      numblocks(mat.GetRowDNums().Size())
  {
    ...
    disjoint_cols = (mat.GetColColoring().Size() == 0);
      ...
      #ifdef CUDA_GRAPH
    struct GraphParams {
      const void* x;   // device pointer to (double*)
      void*       y;   // device pointer to (double*)
      const int*  row; // device pointer to (int*)
      const int*  col; // device pointer to (int*)
      double      s;   // scaling
    };
    #endif
    ...
    MultAdd(double s, const BaseVector & x, BaseVector & y) const
    {
        ...
        uy.UpdateDevice();
          ...
          // persistent graph resources
          static bool            graph_created = false;
          static cudaGraph_t     graph         = nullptr;
          static cudaGraphExec_t graphExec     = nullptr;
          static cudaStream_t    stream        = nullptr;
          static GraphParams*    dparams       = nullptr;     // device control block
        
          // topology/scalar cache (when these change => rebuild/capture)
          static int    cap_nb = -1, cap_W = -1, cap_H = -1;
          static double cap_s  = std::numeric_limits<double>::quiet_NaN();
        
          // persistent temporaries (same addresses across all replays)
          static std::unique_ptr<DevStackArray<double>> dev_hx;
          static std::unique_ptr<DevStackArray<double>> dev_hy;
        
          // problem description for this call
          const int nb = numblocks;
          const int W  = devmat.Width();
          const int H  = devmat.Height();
        
          // device pointers that can vary per call (but we avoid recapture by indirecting through dparams)
          auto xptr    = ux.DevData();
          auto yptr    = uy.DevData();
          auto row_idx = rowdnums.DevData();
          auto col_idx = coldnums.DevData();
        
          // one-time stream and vendor library binding
          if (!stream) {
            cudaStreamCreateWithFlags(&stream, cudaStreamNonBlocking);
            // make sure any vendor libs (if used underneath) use the same stream
            cublasSetStream(Get_CuBlas_Handle(),     stream);
            cusparseSetStream(Get_CuSparse_Handle(), stream);
          }
        
          // Rebuild when topology or baked scalar changes.
          // NOTE: 's' is baked into the expression template (maty = s * ...), so we rebuild if s changes.
          const bool need_rebuild =
              (!graph_created) || (nb != cap_nb) || (W != cap_W) || (H != cap_H) || (s != cap_s);
        
          if (need_rebuild) {
            if (graph_created) {
              cudaGraphExecDestroy(graphExec);
              cudaGraphDestroy(graph);
              graphExec = nullptr; graph = nullptr;
              graph_created = false;
            }
        
            // (Re)allocate persistent temporaries (no alloc during capture)
            dev_hx = std::make_unique<DevStackArray<double>>(static_cast<size_t>(nb) * W);
            dev_hy = std::make_unique<DevStackArray<double>>(static_cast<size_t>(nb) * H);
        
            // allocate device control block once
            if (!dparams) cudaMalloc(&dparams, sizeof(GraphParams));
        
            // ----- CAPTURE -----
            cudaStreamBeginCapture(stream, cudaStreamCaptureModeGlobal);
        
            const size_t NW = static_cast<size_t>(nb) * W;
            const size_t NH = static_cast<size_t>(nb) * H;
        
            // copy-in: hx[tid] = x[row[tid]]   (x/row via dparams)
            DeviceParallelFor(
              NW,
              [locx = dev_hx->DevData(), params = dparams]
              // DEVICE_LAMBDA (size_t tid) {
              //   const GraphParams p = *params;  // load control block once per thread
              //   const double* x     = reinterpret_cast<const double*>(p.x);
              //   const int*    row   = p.row;
              //   locx[tid] = x[row[tid]];
              // }
                DEVICE_LAMBDA (size_t tid) {
                  const GraphParams p = *params;      // was: GraphParams p = *params;
                  const double* x    = reinterpret_cast<const double*>(p.x);
                  const int*    row  = p.row;
                  locx[tid] = x[row[tid]];
                }
            ...
            // matmul: maty = s * matx * Trans(devmat)   (s baked here)
            {
              FlatMatrix<Dev<double>> matx(nb, W, dev_hx->Data());
              FlatMatrix<Dev<double>> maty(nb, H, dev_hy->Data());
              maty = s * matx * Trans(devmat);
            }
        
            // copy-out: atomicAdd(y[col[tid]], hy[tid])   (y/col via dparams)
            DeviceParallelFor(
              NH,
              [locy = dev_hy->DevData(), params = dparams]
            //   DEVICE_LAMBDA (size_t tid) {
            //     const GraphParams p = *params;
            //     double*    y   = reinterpret_cast<double*>(p.y);
            //     const int* col = p.col;
            //     atomicAdd(y + col[tid], locy[tid]);
            //   }
                DEVICE_LAMBDA (size_t tid) {
                  const GraphParams p = *params;      // const
                  double*    y   = reinterpret_cast<double*>(p.y);
                  const int* col = p.col;
                  atomicAdd(y + col[tid], locy[tid]);
                }
            ...
            graph_created = true;
            cap_nb = nb; cap_W = W; cap_H = H; cap_s = s;
          }
        
          // ---- cheap per-call update of device control block (pointers + s for completeness) ----
          GraphParams hparams {
            /*x*/   reinterpret_cast<const void*>((const double*)xptr),
            /*y*/   reinterpret_cast<void*>((double*)yptr),
            /*row*/ row_idx,
            /*col*/ col_idx,
            /*s*/   s
          };
          cudaMemcpyAsync(dparams, &hparams, sizeof(GraphParams), cudaMemcpyHostToDevice, stream);
        
          // replay the captured DAG
        ...
        if (true)
        {
            ...
            tcopyin.Start();
            DeviceParallelFor(numblocks*devmat.Width(),
            ...
            tcopyout.Start();        
            DeviceParallelFor(numblocks*devmat.Height(),
